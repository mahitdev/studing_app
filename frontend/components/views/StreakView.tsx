...
const StreakView: React.FC<StreakViewProps> = ({ dashboard }) => {
  return (
    ...
    >
      ...
      <h2 className="display-lg text-8xl mb-4 font-black">{dashboard?.stats?.streak || 0}</h2>
      ...
      <div className="flex gap-20">
        <div className="text-center">
          ...
          <p className="text-4xl font-black">{dashboard?.stats?.longestStreak || 0}d</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-3">Sync Consistency</p>
          <p className="text-4xl font-black">{dashboard?.stats?.consistencyScore || 0}%</p>
        </div>
      </div>
    </motion.div>
  );
